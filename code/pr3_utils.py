...
def visualize_comparison(pose_imu, mu_imu, pose_slam, mu_slam, show_ori=False):
  fig, ax = plt.subplots(figsize=(5, 5))

  if mu_imu is not None:
      valid_mu_imu = mu_imu[:, mu_imu[2, :] != 0]
      ax.scatter(valid_mu_imu[0, :], valid_mu_imu[1, :], s=1, c='c', alpha=0.2)

  ax.plot(pose_imu[:,0,3], pose_imu[:,1,3], 'g-', linewidth=1.5)
  ax.scatter(pose_imu[-1,0,3], pose_imu[-1,1,3], marker='X', c='g', s=60)

  if mu_slam is not None:
      valid_mu_slam = mu_slam[:, mu_slam[2, :] != 0]
      ax.scatter(valid_mu_slam[0, :], valid_mu_slam[1, :], s=1, c='black', alpha=0.1)

  ax.plot(pose_slam[:,0,3], pose_slam[:,1,3], 'r-', linewidth=1.5)
  ax.scatter(pose_slam[-1,0,3], pose_slam[-1,1,3], marker='o', c='r', s=60)

  ax.scatter(pose_slam[0,0,3], pose_slam[0,1,3], marker='s', c='blue', s=60, zorder=5)

  if show_ori:
      n_pose = pose_slam.shape[0]
      idx = list(range(0, n_pose, max(int(n_pose/50), 1)))
      yaw_list = [mat2euler(pose_slam[i, :3, :3])[2] for i in idx]

      dx, dy = np.cos(yaw_list), np.sin(yaw_list)
      ax.quiver(pose_slam[idx, 0, 3], pose_slam[idx, 1, 3], dx, dy, 
                color="b", units="xy", width=1)
      
  ax.set_title("IMU vs VI-SLAM Trajectory & Landmark")
